...
library(knitr)
opts_chunk$set(warning=FALSE,message=FALSE,cache=TRUE, eval=TRUE, include=TRUE, echo=TRUE, background=rgb(.9,.9,.9), fig.path='img/', dev='quartz_png')
par(mar=c(3.1, 3.1, 3.1, 2.1))
```

# Introduction
The package **`itsadug`** (http://github.com/vr-vr/itsadug) includes several plot functions to visualize the estimates of Generalized Additive (Mixed) Models (GAMM) implemented using the package **`mgcv`** [@Wood_2006; -@Wood_2011]. This paper presents a short overview of:
...
The function **`gamtabs`** converts the summary quickly in a Latex table, which could be included in a `knitr` file. (For the current markdown file the output was translated with pandoc\footnote{\url{http://johnmacfarlane.net/pandoc/}}.)

```{r, eval=FALSE}
gamtabs(m1, caption="Summaty of m1", comment=FALSE)
```
\centering

  : Summaty of m1<span data-label="tab.gam"></span>
...
pvisgam(m1, view=c("Time", "Trial"), select=1, 
     ...
     zlim=c(-15,15), suppressMessages=TRUE)
...
```{r, fvisgam,fig.width=8, fig.height=4, echo=TRUE, include=TRUE,fig.cap="<b>Figure 4.</b> Summed effects surfaces for <code>Time</code> and <code>Trial</code> plotted with <code>fvisgam</code>. Random effects are zeroed out."}
par(mfrow=c(1,2))
# Note: specify zlim when comparing two plots
fvisgam(m1, view=c("Time", "Trial"), 
        cond=list(Group='Children'),
        plot.type='contour', color='topo', main='Group=Children',
        zlim=c(-12,15), suppressMessages=TRUE)
fvisgam(m1, view=c("Time", "Trial"), 
...
```{r, plotSmooth1-code, echo=TRUE, include=FALSE, cache=FALSE, eval=FALSE}
...
abline(h=0) # add horizontal line
```
```{r,plotSmooth1, echo=FALSE, fig.width=8, fig.height=4, include=TRUE, cache=TRUE, fig.cap="<b>Figure 5.</b> Using <code>plot.gam</code> for plotting a one dimensional smooth."}
par(mfrow=c(1,2), cex=1.1)
# As there is no one-dimensional smooth in model m1,
# the interaction of m1 is decomposed in smooths and 
# partial interactions:
...
plot_smooth(m2, view="Time", cond=list(Group="Adults"), rug=FALSE, 
  ylim=c(-10,15), suppressMessages=TRUE,
...
plot_smooth(m2, view="Time", cond=list(Group="Adults"), 
  ...
  ylim=c(-10,15), suppressMessages=TRUE, xpd=TRUE)
# Add legend:
legend('bottomleft', 
...
plot_smooth(m1, view="Time", cond=list(Group="Adults"), 
  rug=FALSE, ylim=c(-10,15), suppressMessages=TRUE,
...
par(mfrow=c(1,2))
plot_diff2(m1, xvar="Time", yvar="Trial", 
        by_predictor="Group", 
        comp_levels=c("Adults", "Children"),
        zlim=c(-7.5,5),
        main='Difference Adults-Children')
plot_diff(m1, xvar="Time",         
        by_predictor="Group", 
        comp_levels=c("Adults", "Children"),
...
```{r, predict1, echo=TRUE, cache=TRUE, include=TRUE}
...
```{r, predict2, echo=TRUE, cache=TRUE, include=TRUE}
# Extract prediction from model
# Note that the random effects are canceled out by setting rm.ranef to TRUE
xval <- seq(0,2000, length=100)
...
```{r,predictRandom, echo=TRUE, cache=TRUE, include=TRUE}
...
```{r, random, echo=TRUE, fig.width=8, fig.height=4, include=TRUE, cache=FALSE, fig.cap="<b>Figure 11.</b> <i>Left:</i> Random effects smooths. <i>Right:</i> The mean (black) and median (blue) of the random smooths."}
par(mfrow=c(1,2))
